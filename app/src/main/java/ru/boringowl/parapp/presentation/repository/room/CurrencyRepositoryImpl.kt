...
class CurrencyRepositoryImpl : CurrencyRepository {
    ...
    init {
        ...
        currencyDAO = db.exchangeRateDAO()
        allRates = currencyDAO.getAllRates() as LiveData<List<CurrencyRate>>
    }

    override fun <T : CurrencyRate> getAllRates(): LiveData<List<T>> {
        return allRates as LiveData<List<T>>
    }
    ...
    override fun <T : CurrencyRate> getRate(pair: Pair<String, String>, onError: (message: String) -> Unit): LiveData<T?> {
        val currency: LiveData<CurrencyRate?> = liveData {
            try {
                val local = currencyDAO.getRateSync(pair)

                if (local == null || local.nextUpdate.before(java.sql.Timestamp(System.currentTimeMillis() / 1000))) {
                ...
                emitSource(
                    currencyDAO.getRate(pair) as LiveData<CurrencyRate?>
                )
